...
$(document).ready(function() {
	...
	$("[id^=mini_profiler_result_]").live("click", function() {
		var resultId = this.id.substr("mini_profiler_result_".length, this.id.length);
		var resultDetails = "#mini_profiler_result_details_" + resultId;

		$(resultDetails).toggle();
		$(".mini-profiler-result-details").not(resultDetails).hide();
	});
});